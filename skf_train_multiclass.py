...
def getDataloader(dataset, train_idx, valid_idx, batch_size, num_workers):
    # ?¸?ë¡? ? „?‹¬ë°›ì?? dataset?—?„œ train_idx?— ?•´?‹¹?•˜?Š” Subset ì¶”ì¶œ
    train_set = torch.utils.data.Subset(dataset,
                                        indices=train_idx)
    # ?¸?ë¡? ? „?‹¬ë°›ì?? dataset?—?„œ valid_idx?— ?•´?‹¹?•˜?Š” Subset ì¶”ì¶œ
    val_set = torch.utils.data.Subset(dataset,
                                      indices=valid_idx)

    # ì¶”ì¶œ?œ Train Subset?œ¼ë¡? DataLoader ?ƒ?„±
    ...
    # ì¶”ì¶œ?œ Valid Subset?œ¼ë¡? DataLoader ?ƒ?„±
...
def grid_image(np_images, gts, preds, n=16, shuffle=False):
    ...
    # cautions: hardcoded, ?´ë¯¸ì?? ?¬ê¸°ì— ?”°?¼ figsize ë¥? ì¡°ì •?•´?•¼ ?•  ?ˆ˜ ?ˆ?Šµ?‹ˆ?‹¤. T.T
    figure = plt.figure(figsize=(12, 18 + 2))
    # cautions: hardcoded, ?´ë¯¸ì?? ?¬ê¸°ì— ?”°?¼ top ë¥? ì¡°ì •?•´?•¼ ?•  ?ˆ˜ ?ˆ?Šµ?‹ˆ?‹¤. T.T
...
def initialize_weights(model):
    """
    Xavier uniform ë¶„í¬ë¡? ëª¨ë“  weight ë¥? ì´ˆê¸°?™”?•©?‹ˆ?‹¤.
    ?” ë§ì?? weight ì´ˆê¸°?™” ë°©ë²•??? ?‹¤?Œ ë¬¸ì„œ?—?„œ ì°¸ê³ ?•´ì£¼ì„¸?š”. https://pytorch.org/docs/stable/nn.init.html
    """
...
def train(data_dir, model_dir, args):
    ...
    # public, private ?…Œ?Š¤?Š¸?…‹?´ ì¡´ì¬?•˜?‹ˆ ê°ê°?˜ ?˜ˆì¸¡ê²°ê³¼ë?? ????¥?•©?‹ˆ?‹¤.
    # meta ?°?´?„°??? ?´ë¯¸ì?? ê²½ë¡œë¥? ë¶ˆëŸ¬?˜µ?‹ˆ?‹¤.
    submission = pd.read_csv(os.path.join(test_img_root, 'info.csv'))
    test_image_dir = os.path.join(test_img_root, 'images')
    # Test Dataset ?´?˜?Š¤ ê°ì²´ë¥? ?ƒ?„±?•˜ê³? DataLoaderë¥? ë§Œë“­?‹ˆ?‹¤.
    ...
    train_log_interval = 20  # logging?•  iteration?˜ ì£¼ê¸°
    ...
    # K-Fold Cross Validationê³? ?™?¼?•˜ê²? Train, Valid Indexë¥? ?ƒ?„±?•©?‹ˆ?‹¤.
        ...
        for epoch in range(args.epochs):
            ...
            for idx, train_batch in enumerate(train_loader):
                ...
                if (idx + 1) % train_log_interval == 0:
                    ...
                    loss_value = 0
                    matches = 0
            # wandb ?•™?Šµ ?‹¨ê³„ì—?„œ Loss, Accuracy ë¡œê·¸ ????¥
            wandb.log({
            ...
            with torch.no_grad():
                ...
                # mask, genderm age (?‰ê·? loss,acc ê³„ì‚°)
                ...
                # one class loss, acc ê³„ì‚°

                val_loss = np.sum(val_loss_items) / len(val_loader)
                val_acc = np.sum(val_acc_items) / len(valid_idx)

                # Callback1: validation accuracyê°? ?–¥?ƒ? ?ˆ˜ë¡? ëª¨ë¸?„ ????¥?•©?‹ˆ?‹¤.
...
#                     torch.save(model.state_dict(), f"{save_dir}/results/{epoch:03}_accuracy_{val_acc:4.2%}.ckpt")
                ...
                print()
                # wandb ê²?ì¦? ?‹¨ê³„ì—?„œ Loss, Accuracy ë¡œê·¸ ????¥
                wandb.log({
                    "valid_loss": val_loss,
                    "valid_acc": val_acc,
                ...
                # Callback2: patience ?šŸ?ˆ˜ ?™?•ˆ ?„±?Š¥ ?–¥?ƒ?´ ?—†?„ ê²½ìš° ?•™?Šµ?„ ì¢…ë£Œ?‹œ?‚µ?‹ˆ?‹¤.
                if counter > patience:
                    print("Early Stopping...")
                    break

        # ê°? fold?—?„œ ?ƒ?„±?œ ëª¨ë¸?„ ?‚¬?š©?•´ Test ?°?´?„°ë¥? ?˜ˆì¸¡í•©?‹ˆ?‹¤.
        all_predictions = []
        with torch.no_grad():
            for images in test_loader:
                ...
                pred = model(images) / 2  # ?›ë³? ?´ë¯¸ì??ë¥? ?˜ˆì¸¡í•˜ê³?
                # horizontal_flip?œ¼ë¡? ?’¤ì§‘ì–´ ?˜ˆì¸¡í•©?‹ˆ?‹¤.
                pred += model(torch.flip(images, dims=(-1,))) / 2
                all_predictions.extend(pred.cpu().numpy())

            fold_pred = np.array(all_predictions)

        # ?™•ë¥? ê°’ìœ¼ë¡? ?•™?ƒë¸”ì„ ì§„í–‰?•˜ê¸? ?•Œë¬¸ì— 'k'ê°œë¡œ ?‚˜?ˆ„?–´ì¤ë‹ˆ?‹¤.
        if oof_pred is None:
