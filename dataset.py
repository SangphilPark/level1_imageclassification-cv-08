...
class MaskBaseDataset(Dataset):
    ...
    def __getitem__(self, index):
        assert self.transform is not None, ".set_tranform ë©”ì†Œ?“œë¥? ?´?š©?•˜?—¬ transform ?„ ì£¼ìž…?•´ì£¼ì„¸?š”"
...
class MaskMultiLabelDataset(Dataset):
    ...
    def setup(self):
        profiles = os.listdir(self.data_dir)
        for profile in profiles:
            if profile.startswith("."):  # "." ë¡? ?‹œ?ž‘?•˜?Š” ?ŒŒ?¼??? ë¬´ì‹œ?•©?‹ˆ?‹¤
                continue

            img_folder = os.path.join(self.data_dir, profile)
            for file_name in os.listdir(img_folder):
                _file_name, ext = os.path.splitext(file_name)
                if _file_name not in self._file_names:  # "." ë¡? ?‹œ?ž‘?•˜?Š” ?ŒŒ?¼ ë°? invalid ?•œ ?ŒŒ?¼?“¤??? ë¬´ì‹œ?•©?‹ˆ?‹¤
                    continue
...
#         multi_class_label = self.encode_multi_class(mask_label, gender_label, age_label)
    ...
    def split_dataset(self) -> Tuple[Subset, Subset]:
        """
        ?°?´?„°?…‹?„ train ê³? val ë¡? ?‚˜?ˆ•?‹ˆ?‹¤,
        pytorch ?‚´ë¶??˜ torch.utils.data.random_split ?•¨?ˆ˜ë¥? ?‚¬?š©?•˜?—¬
        torch.utils.data.Subset ?´?ž˜?Š¤ ?‘˜ë¡? ?‚˜?ˆ•?‹ˆ?‹¤.
        êµ¬í˜„?´ ?–´? µì§? ?•Š?œ¼?‹ˆ êµ¬ê??ë§? ?˜¹??? IDE (e.g. pycharm) ?˜ navigation ê¸°ëŠ¥?„ ?†µ?•´ ì½”ë“œë¥? ?•œ ë²? ?½?–´ë³´ëŠ” ê²ƒì„ ì¶”ì²œ?“œë¦½ë‹ˆ?‹¤^^
        """
        n_val = int(len(self) * self.val_ratio)
        n_train = len(self) - n_val
        train_set, val_set = random_split(self, [n_train, n_val])
